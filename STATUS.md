...
| G6 trusted release | Runtime cache preparation repair in progress | Worker commits `6ce2c3b` / `f01318e` and Windows/Ubuntu CI runs `31188419241` / `31191145020` pass. `final-v5` prepared the fixed toolchain, firmware and both pinned QEMU rootfs assets, then exposed a Windows CRLF conversion failure before QEMU. The Worker now uses a short Windows root, long-path Git options and LF checkout. `final-v8` validated the fixed toolchain, all firmware and Cargo build but failed before QEMU because its generated workspace overlay was not materialized into the preparation worktree. `final-v9` adds that verified replay before building; formal mount S0, pids S2 and seeded-fault S3 remain unresolved. |
