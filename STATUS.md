...
| G6 trusted release | Worker mount strategy repaired; clean S0 rerun pending | The fixed runtime cache is complete and verified. The prior formal result stopped before QEMU because Rustup tried to update inside a read-only container. Commit `697f41e` mounts toolchain, Rustup, QEMU images, firmware metadata and workspace overlay individually as read-only, keeps only `target` writable, and pins `RUSTUP_TOOLCHAIN`; this avoids copying the 8.66GB cache. A stale old Node invocation was stopped after it wrote a 137 result using the pre-fix `runtime-ro` command. Local `npm run check` remains 53/53. Formal mount S0, pids S2 and seeded-fault S3 remain unresolved until a clean replay produces real QEMU evidence. |
