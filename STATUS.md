...
| G6 trusted release | S0 reaches QEMU; longer replay pending | The fixed runtime cache is complete and verified. Toolchains and overlays stay read-only; disposable copies contain only compiled target and QEMU images that require lock files. Full 40,885-file verification takes about 195 seconds and preserves root hash `50ce18e...`. With 256 processes/8GB, StarryOS release builds and both rootfs checks pass; the Worker reaches `[1/1] starry qemu system`, but the 900-second baseline expires during QEMU run-directory preparation before cgroup-basic emits results. Windows cleanup now retries transient `ENOTEMPTY`. Local regression is 54/54. Formal S0, pids S2 and seeded-fault S3 remain unresolved. |
