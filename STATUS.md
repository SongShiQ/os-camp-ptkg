...
| G6 trusted release | Formal S0 unresolved | The fixed runtime cache is complete and verified. Toolchains and overlays stay read-only; disposable copies contain only compiled target and QEMU images that require lock files. Full 40,885-file verification takes about 195 seconds and preserves root hash `50ce18e...`. The latest persisted formal result is still the 900-second replay: StarryOS release builds and both rootfs checks pass, then the Worker reaches `[1/1] starry qemu system` but times out during QEMU run-directory preparation before cgroup-basic emits results. A later 2,400-second attempt did not produce a new execution-result and left a disposable Worker residue, so it cannot replace the persisted evidence. Worker cleanup now runs in a scope-checked child process with a five-minute hard timeout, preventing Windows NTFS/reparse-point cleanup from indefinitely blocking result persistence. Current authoring validation reports four execution/sandbox blockers. Formal S0, pids S2 and seeded-fault S3 remain unresolved. |

Current regression baseline:

- PTKG: 56 tests, typecheck passing locally on Node 24 (rechecked 2026-08-17).
...
- Current cgroup authoring run: 15 fixed-source facts, 5 behavior chains, 6 learning slices and 27/27 verified anchors. Core hierarchy, membership transactions and controller registry/dispatch now have S1 design-review slices with fixed rubrics and no fake execution references; mount S0 and pids S2/S3 remain the executable slices. Projection validation passes with 21 nodes, 29 edges and 27 sources. Authoring validation still has exactly four blockers because mount S0, pids S2 and seeded-fault S3 lack succeeded execution results and the persisted S0 cleanup/reset did not succeed. The split is reviewable, not release-ready.
