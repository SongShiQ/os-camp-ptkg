...
The frozen tgoskits image is not yet available in the current Docker cache and is being downloaded by one background pull, so real mount S0, pids S2 and seeded-fault S3 evidence remains unresolved. The three succeeded results in `fixtures/authoring/cgroup-golden` demonstrate the data contract only; a floating tag or fixture result must not be substituted for the release gate.
